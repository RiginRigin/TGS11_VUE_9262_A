<template>
  <v-container>
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        <v-card-title>
          <span class="headline">Input Sparepart</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12">
                <v-select :items="jenis" label="Merk" v-model="form.merk"></v-select>
              </v-col>
              <v-col cols="12">
...
export default {
  ...
  methods: {
    ...
    editHandler(item) {
      ...
      (this.form.amount = item.amount), (this.updatedId = item.id);
    },
    deleteData(deleteId) {
      //menghapus data
